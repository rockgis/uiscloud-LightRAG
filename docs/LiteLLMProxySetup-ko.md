...
```python
# AWQ 모델 garbage 감지용 패턴 (모듈 레벨에 정의)
_GARBAGE_PATTERNS = [
...
2. `_trim_garbage_tail()`로 본문 끝의 garbage 제거 (두 가지 모델 패턴 처리)
...
| 모델 | garbage 특성 | 감지 방법 |
|------|------------|---------|
| Qwen3-AWQ | 수학기호·그리스·키릴 문자 순수 나열 | exotic 비율 > 25% + Korean < 10% |
| EXAONE-AWQ | 한국어 + ASCII 혼합 워드 샐러드 (`!!!!`, `\|\|`, `ALLCAPS`, `___`) | `_GARBAGE_PATTERNS` 2개 이상 매칭 |

이 수정은 `commit 35a5a66` + garbage trim 추가에서 적용되었으며 `knowwheresoft/uiscloud-lightrag:1.5.0` 이미지의 서버 override 파일에 포함됩니다.
...
**원인:** EXAONE-4.5-33B-AWQ 모델이 `max_tokens`에 도달하면 한국어 + 무작위 ASCII 식별자를 혼합한 워드 샐러드를 생성합니다. Qwen3-AWQ의 순수 exotic 문자 garbage와 다르게 한국어가 섞여 있어 단순 exotic 비율 감지가 실패합니다.

EXAONE garbage 예시:
```
포함됨!!!!.]--|[포함됨 영문 회계 튜토리얼 원고|추정 없음||RAes:k_i_d_f_c____특이코스포바이DESCRIPT_EVALDENG
```

**해결 1 (코드 수정 — 영구):** `_trim_garbage_tail()` 함수에 EXAONE-style 패턴 감지를 추가합니다 (위의 "References 파일명 번역/변형" 해결책 참고).

감지 조건: **40자 이상** + **한국어 포함** + 아래 패턴 **2개 이상** 매칭:
...
```bash
# Docker 컨테이너 내부에서 초기화 후 재시작
docker exec lightrag-lightrag-1 sh -c "echo '{}' > /app/data/rag_storage/kv_store_llm_response_cache.json"
