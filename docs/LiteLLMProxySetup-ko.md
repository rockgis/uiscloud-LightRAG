...
```python
# AWQ 모델 garbage 감지용 패턴 (모듈 레벨에 정의, commit 3f51427 기준 10개)
_GARBAGE_PATTERNS = [
    ...
    re.compile(r"[,;]{3,}"),             # 3회 이상 연속된 쉼표/세미콜론 (global mode)
    re.compile(r"""["']{3,}"""),          # 3회 이상 연속된 따옴표 (hybrid mode)
    re.compile(r"\\{2,}"),               # 2회 이상 연속된 역슬래시 (hybrid mode)
    re.compile(r"[—–]{2,}"),             # 2회 이상 연속된 em/en 대시 (local mode)
]
...
def _trim_garbage_tail(body: str) -> str:
    ...
    for line in lines:
        ...
        # EXAONE-style: 40자 이상이고 한국어 포함 + 코드 정크 패턴 2개 이상 매칭
...
| EXAONE-AWQ (초기) | 한국어 + ASCII 혼합 워드 샐러드 | `_GARBAGE_PATTERNS` 2개 이상 매칭 | `포함됨!!!!.\|--\|DESCRIPT_EVALDENG` |
| EXAONE-AWQ (강화) | 쿼리 모드별 다양한 garbage 패턴 | 추가 패턴 10개로 감지 강화 | global: `So,,,,,,,,`, hybrid: `config"""=`, local: `————___` |
| EXAONE-AWQ (naive) | 영어 word-salad (순수 ASCII) | 500자+ 줄, 한국어 < 5% | `epistemological debates axiological judgments teleos-emotional...` |

**추가된 garbage 패턴 (commit 3f51427, 4가지 모드 실제 테스트로 검증):**
...
이 수정은 `commit 35a5a66` + `commit a9970c1` + `commit 3f51427`에서 순차 적용되었으며 `knowwheresoft/uiscloud-lightrag:1.5.0` 이미지의 서버 override 파일에 포함됩니다.
...
**해결 1 (코드 수정 — 영구):** `_trim_garbage_tail()` 함수에 EXAONE-style 패턴 감지 (총 10개 패턴, 현재 적용 중):

EXAONE-style 감지 조건: **40자 이상** + **한국어 포함** + 아래 패턴 **2개 이상** 매칭:

| 패턴 | 설명 |
|------|------|
| `[A-Za-z]{3,}[가-힣]` | 영어가 공백 없이 한국어에 직접 연결 |
| `[가-힣][A-Za-z]{3,}` | 한국어가 공백 없이 영어에 직접 연결 |
| `[!?~]{3,}` | 3회 이상 반복 구두점 |
| `\|{2,}` | 2회 이상 연속 파이프 |
| `[A-Z_]{5,}` | 5자 이상 ALL_CAPS_IDENTIFIER |
| `[_]{3,}` | 3회 이상 연속 언더스코어 |
| `[,;]{3,}` | 3회 이상 연속 쉼표/세미콜론 |
| `["']{3,}` | 3회 이상 연속 따옴표 |
| `\\{2,}` | 2회 이상 연속 역슬래시 |
...
영어 word-salad 감지 조건 (naive mode): **500자 이상** + **한국어 포함** + **한국어 비율 < 5%**
