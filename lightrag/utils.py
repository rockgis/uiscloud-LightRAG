        ...
        if llm_timeout is not None:
            ...
            if max_execution_timeout is None:
                max_execution_timeout = (
                    llm_timeout * 8
                )  # Allow multiple retries (5x) with wait time within worker budget
...
def remove_think_tags(text: str) -> str:
    ...
    3. Truncated <think> block with no closing </think> (timeout/max_tokens):
       return empty string so callers can retry.
    """
    ...
    text = re.sub(r"<think>.*?</think>", "", text, flags=re.DOTALL)
    # If a <think> remains with no closing tag (truncated thinking), discard it
    if "<think>" in text and "</think>" not in text:
        pre_think = text[: text.index("<think>")].strip()
        return pre_think  # Return content before the truncated block (may be empty)
    return text.strip()
